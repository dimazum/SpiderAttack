...
public class PickUpgradeController : BaseUpgradeController
{
    [SerializeField]
    ...
    {
        firstShopImage.sprite = models.GetChild(lvl).GetComponent<PickUpgradeModel>().sprite;
        if (GameStates.PickLvl < 4)
        {
            secondShopImage.sprite = models.GetChild(lvl + 1).GetComponent<PickUpgradeModel>().sprite;
        }
        else
        {
    ...
    protected override void RenderPrice(int lvl)
    {
        if (GameStates.PickLvl < 4)
        {
            priceText.text = models.GetChild(lvl + 1).GetComponent<PickUpgradeModel                                         >().price.ToString();
        }
    }
    ...
    {
        charakterPick.sprite = models.GetChild(lvl).GetComponent<PickUpgradeModel>().sprite;
    }
}
